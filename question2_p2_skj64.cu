...
int dataIndex;
for (dataIndex = tid+1; dataIndex < N; dataIndex++){
    if (tid != dataIndex){
        if ((((inputData[tid].x - inputData[dataIndex].x) * (inputData[tid].x - inputData[dataIndex].x)) + 
        ((inputData[tid].y - inputData[dataIndex].y) * (inputData[tid].y - inputData[dataIndex].y))) <= (*inputEpsilon) * (*inputEpsilon)){
            atomicAdd(numOfDistancesWithinEps, int(2));
        }
